...
const AdminProduct = () => {
  ...
  const { data: products, isLoading: isLoadingProducts, refetch: refetchProducts } = useQuery(
    ...
    async () => {
      if (typeFilter) return await ProductService.getProductType(typeFilter, 1, 100);
      return await ProductService.getAllProduct(search);
    }
  );

  const { data: allTypes } = useQuery(['types'], ProductService.getAllTypeProduct, {
    onSuccess: (data) => setTypes(data?.data || []),
  });
  ...
  const handleEditProduct = async (id) => {
    ...
    try {
      ...
      if (product) {
        ...
        setRowSelected(id);
      } else message.error('Không tìm thấy sản phẩm!');
    } catch (err) {
      console.error(err);
