import { Button, Form, Upload, Select, Popconfirm, Row, Col } from 'antd';
...
const AdminProduct = () => {
  ...
  const { data: products, isLoading: isLoadingProducts, refetch: refetchProducts } = useQuery({
    ...
    queryFn: async () => {
      if (filterType) {
        const res = await ProductService.getProductType(filterType, 1, 1000);
        return res;
      } else {
  ...
  const { data: types, refetch: refetchTypes } = useQuery({
    ...
    queryFn: async () => await ProductService.getAllTypeProduct(),
    onSuccess: (data) => setProductTypes(data?.data || []),
  });
  ...
  const deleteManyMutation = useMutation({
    ...
    onError: () => message.error('Xóa nhiều sản phẩm thất bại!'),
  });
  ...
  const renderAction = (record) => (
    <div style={{ display: 'flex', gap: 8 }}>
      <EditOutlined
        style={{ color: 'orange', fontSize: 20, cursor: 'pointer' }}
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginBottom: 16, alignItems: 'center' }}>
        ...
        <Col>
          <Button type="danger" icon={<DeleteOutlined />} onClick={handleDeleteMany} disabled={selectedRowKeys.length === 0}>
            Xóa nhiều
          </Button>
        </Col>
        <Col>
          ...
          >
            ...
            <Form.Item label="Loại" name="type">
              <Select
                ...
                value={stateProductDetails.type || undefined}
            ...
            <Form.Item label="Thêm loại mới" name="newType">
              <InputComponent
                ...
                name="newType"
                placeholder="Nhập loại mới nếu muốn"
              />
            </Form.Item>
              ...
              >
                ...
                {stateProductDetails.image && (
                  <img
                    ...
                    alt="product"
                    style={{ height: 60, width: 60, objectFit: 'cover', marginLeft: 10 }}
                  />
                )}
              </WrapperUploadFile>
            </Form.Item>
