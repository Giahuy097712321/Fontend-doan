import { Button, Form, Upload, Input, Select, Tag } from 'antd';
...
const AdminProduct = () => {
  ...
  const mutationUpdate = useMutationHooks((data) => ProductService.updateProduct(data.id, user?.access_token, data));
  const mutationDeleteMany = useMutationHooks((data) => ProductService.deleteManyProduct(data.ids, data.token));
  const mutationDeleteSingle = useMutationHooks((id) => ProductService.deleteProduct(id, user?.access_token));

  // Lấy tất cả loại sản phẩm
  useEffect(() => {
    ProductService.getAllTypeProduct().then((res) => setTypes(res?.data || []));
  }, []);

  // Query sản phẩm
  const fetchProducts = async ({ page, pageSize }) => {
    const res = await ProductService.getAllProduct(searchText, pageSize);
    return res;
  };

  const { data: products, refetch, isLoading: isLoadingProducts } = useQuery(
    ['products', searchText],
    () => fetchProducts({ page: pagination.current, pageSize: pagination.pageSize }),
  ...
  const handleDeleteSingle = (id) => {
    mutationDeleteSingle.mutate(id, {
      onSuccess: () => {
        message.success('Xóa sản phẩm thành công!');
        refetch();
      },
  ...
  const renderAction = (record) => (
    <div style={{ display: 'flex', gap: 8 }}>
      ...
      <DeleteOutlined style={{ color: 'red', fontSize: '20px', cursor: 'pointer' }} onClick={() => handleDeleteSingle(record._id)} />
    </div>
  );

  const renderView = (record) => (
    <EyeOutlined style={{ color: 'blue', fontSize: '20px', cursor: 'pointer' }} onClick={() => handleEditProduct(record._id)} />
  );
  ...
  const columns = [
    ...
    { title: 'Loại', dataIndex: 'type' },
    {
      ...
      render: (count) => (count === 0 ? <Tag color="red">Hết hàng</Tag> : count),
    },
    { title: 'Giá', dataIndex: 'price' },
    { title: 'Discount (%)', dataIndex: 'discount' },
    {
      title: 'Giá sau giảm',
      render: (_, record) => (record.price * (1 - record.discount / 100)).toLocaleString(),
    },
  ...
  return (
    <div>
      ...
      <Form layout="inline" style={{ marginBottom: 16 }}>
        <Form.Item>
          <Input placeholder="Tìm theo tên sản phẩm" value={searchText} onChange={(e) => setSearchText(e.target.value)} />
        </Form.Item>
        <Form.Item>
          <Select
          ...
          <Form name="product" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} onFinish={onSaveProduct} form={form} autoComplete="off">
            ...
            <Form.Item label="Hình ảnh" name="image">
              <WrapperUploadFile onChange={handleOnchangeImage} maxCount={1} showUploadList={false} beforeUpload={() => false}>
                ...
                {stateProductDetails.image && <img src={stateProductDetails.image} alt="product" style={{ height: 60, width: 60, objectFit: 'cover', marginLeft: 10 }} />}
              </WrapperUploadFile>
            </Form.Item>
