...
import { parsePrice, getDataChart } from '../../utils';
...
const AdminOrder = () => {
   ...
   const handleEditOrder = (orderId) => {
    ...
    const totalRevenue = useMemo(() => {
        ...
        return orders.data.reduce((sum, o) => sum + parsePrice(o.totalPrice || 0), 0);
    }, [orders]);

    // Dữ liệu chart
    const dataChart = useMemo(() => getDataChart(orders?.data), [orders]);
    ...
    const columns = [
        ...
        { title: 'Sản phẩm', dataIndex: 'products', ellipsis: true },
        {
            title: 'Hành động',
            render: (_, record) => (
                <EditOutlined style={{ color: 'orange', fontSize: '18px', cursor: 'pointer' }} onClick={() => handleEditOrder(record)} />
            ),
        },
    ];
