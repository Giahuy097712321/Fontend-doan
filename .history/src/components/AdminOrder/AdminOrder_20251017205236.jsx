...
const AdminOrder = () => {
    ...
    // 🟢 Dữ liệu bảng
    // 🟢 Dữ liệu bảng (dataTable)
    let dataTable = [];

    // Kiểm tra orders và orders.data có tồn tại không
    if (orders && orders.data) {
        // Giả sử API trả về orders.data là mảng
        // Nếu API của bạn khác, hãy chỉnh đúng đường dẫn
        const listOrders = Array.isArray(orders.data) ? orders.data : [];

        dataTable = listOrders.map((item) => ({
            key: item._id,
    ...
    // Nếu orders.data chưa về hoặc không phải mảng, dataTable vẫn là []
