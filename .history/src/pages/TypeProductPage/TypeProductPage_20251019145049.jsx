...
const TypeProductPage = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // 🔍 Nếu có danh mục (vd: “Tủ lạnh”) thì lấy tất cả và lọc thủ công
      const allProducts = await ProductService.getAllProduct('', paginate.limit * 10)
      if (allProducts?.status === 'OK') {
        let filtered = allProducts.data

        if (state) {
          filtered = filtered.filter(
            (p) => p.type?.toLowerCase() === state.toLowerCase()
          )
        }
