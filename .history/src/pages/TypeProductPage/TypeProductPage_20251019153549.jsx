...
const TypeProductPage = () => {
  ...
  const fetchProductType = async (type, page, limit) => {
    ...
    try {
      ...
      if (res?.status === 'OK') {
        ...
        console.log('🔍 Requested category:', type)
        console.log('📊 All products received:', res?.data)
        console.log('🏷️ Product types:', [...new Set(res?.data.map(p => p.type))])
  ...
  useEffect(() => {
    if (allProducts.length > 0) {
      ...
      console.log('Current category from URL:', state)

      const productsByType = allProducts.reduce((acc, product) => {
        acc[product.type] = (acc[product.type] || 0) + 1
        return acc
      }, {})
      console.log('Products count by type:', productsByType)
      ...
      allProducts.forEach((product, index) => {
        console.log(`Product ${index + 1}:`, {
          name: product.name,
          type: product.type,
          matchesCategory: product.type === state
        })
      })
    }
  }, [allProducts, state])
  ...
  const applyFiltersAndSort = () => {
    ...
    console.log('Initial products:', result.length)
    ...
    console.log(`💰 Price filter: ${beforePrice} → ${result.length}`)
    ...
    if (selectedRating > 0) {
      ...
      console.log(`⭐ Rating filter: ${beforeRating} → ${result.length}`)
    }
    ...
    console.log('Product names:', result.map(p => p.name))
  ...
  useEffect(() => {
    fetchAllTypeProduct()
    if (state) {
      console.log('🚀 Fetching products for category:', state)
